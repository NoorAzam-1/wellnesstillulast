...
const INITIATIVES_DATA = [
  {
    ...
    longDesc: "Our Carbon Garden projects leverage high-sequestration plant species curated specifically to maximize natural CO2 absorption. By utilizing organic composting, urban soil optimization, and native flora, we turn unused urban plots into highly efficient carbon-capturing micro-ecosystems.",
    icon: "CO2",
    ...
    keyBenefit: "Utilizes multi-layered canopy structures to maximize vertical carbon absorption density by 4x."
  },
  {
    id: "miyawaki-forest",
    ...
    longDesc: "The Miyawaki method replicates natural ecosystem dynamics by planting dozens of native species close together. This creates self-sustaining, multi-tiered forests that grow up to 10 times faster, are 30 times denser, and house 100 times more biodiversity than conventional plantations.",
    icon: "Sprout",
    ...
    keyBenefit: "Restores local bird, insect, and soil microbial life while serving as excellent localized air filtration blocks."
  },
  {
    id: "stp-sewage",
    title: "STP Sewage Treatment",
    category: "Water Solutions",
    shortDesc: "Advanced sewage treatment plants ensuring clean water is returned to nature safely.",
    longDesc: "Our modern Sewage Treatment Plants (STP) implement biological purification paired with advanced multi-tier filtration. By omitting toxic chemicals, we cycle graywater back into local ground reservoirs and agricultural channels completely pathogen-free and nutrient-rich.",
    icon: "Shield",
    iconBg: "bg-blue-50 text-blue-700 border-blue-100",
    image: "https://images.unsplash.com/photo-1541535881962-e668f2244a26?auto=format&fit=crop&q=80&w=800",
    stats: { primary: "4.5M Liters", label: "Daily Recycled Water" },
    keyBenefit: "Decentralized modular setups that operate under 35% lower energy profiles compared to municipal baselines."
  },
  {
    id: "water-conservation",
    title: "Water Conservation",
    category: "Water Solutions",
    shortDesc: "Conserving every drop through innovative solutions, rainwater harvesting, and usage tracking.",
    longDesc: "Water Conservation tackles modern dry seasons through localized smart catchments, groundwater recharge wells, and low-waste flow systems. We build resilient local rain reservoirs that prevent water table depletion and minimize surface runoff erosion.",
    icon: "Droplet",
    ...
    keyBenefit: "Implements gravel-charcoal aquifer filters that pre-treat collected rainfall back to drinkable standard safely."
  },
  {
    id: "microgreen-farming",
    title: "Microgreen Farming",
    category: "Sustainable Food",
    shortDesc: "Promoting healthy living through sustainable microgreen farming and urban agriculture.",
    longDesc: "Microgreens contain up to 40 times higher nutrient densities than their fully mature counterparts. We cultivate organic superfoods inside compact vertical systems using 90% less land and water than traditional agriculture, bringing fresh nutrition directly to urban communities.",
    icon: "Leaf",
    ...
    keyBenefit: "Zero pesticides or chemical runoff, utilizing nutrient-rich organic compost bases from carbon gardens."
  },
  {
    id: "herbal-nursery",
    title: "Herbal Nursery",
    category: "Sustainable Food",
    shortDesc: "Growing a wide variety of medicinal and aromatic plants for a healthier tomorrow.",
    longDesc: "Our Herbal Nursery preserves, propagates, and distributes vital therapeutic and culinary plants. We empower communities to reduce dependency on industrial pharmaceuticals by providing access to homegrown remedies, native plants, and botanical wellness guidance.",
    icon: "Pot",
    ...
    keyBenefit: "Protects and re-introduces highly endangered local herbal flora into residential and suburban parks."
  }
];

const CATEGORIES = ["All", "Land & Forests", "Water Solutions", "Sustainable Food"];
...
export default function HeroSection() {
  ...
  const [calcTrees, setCalcTrees] = useState(2);
  const [calcWaterDays, setCalcWaterDays] = useState(5);
  const [calcWasteSaved, setCalcWasteSaved] = useState(10);
  
  
  // Custom interactive user details for "Get Involved"
  ...
  const filteredInitiatives = useMemo(() => {
    ...
    return INITIATIVES_DATA.filter(item => item.category === selectedCategory);
  }, [selectedCategory]);

  
  const waterConservedTotal = useMemo(() => {
    return (calcWaterDays * 150).toFixed(0);
  }, [calcWaterDays]);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <header className="text-center mb-8 relative">
          ...
          <p className="mt-6 text-base sm:text-lg text-gray-600 max-w-4xl mx-auto leading-relaxed">
            Explore our key initiatives that contribute to environmental conservation, 
            resource management, and a sustainable, climate-resilient future.
          </p>
        </header>
            ...
            >
              ...
              <div className="relative h-64 overflow-visible bg-emerald-50">
                <img 
                  ...
                  loading="lazy"
                  onError={(e) => {
                    e.target.src = "https://images.unsplash.com/photo-1513836279014-a89f7a76ae86?q=80&w=800";
                  }}
                />
                ...
                <div className="absolute -bottom-8 left-8 w-16 h-16 rounded-full bg-white flex items-center justify-center shadow-xl border border-[#dde8d8] z-10">
                  <div className="w-12 h-12 rounded-full bg-[#f1f8ee] flex items-center justify-center text-[#1f7d2d]">
                    {item.icon === "CO2" && <span className="font-extrabold text-xs">CO₂</span>}
                    {item.icon === "Sprout" && <Sprout className="w-6 h-6" />}
                    {item.icon === "Shield" && <ShieldCheck className="w-6 h-6" />}
                    {item.icon === "Droplet" && <Droplet className="w-6 h-6" />}
                    {item.icon === "Leaf" && <Leaf className="w-6 h-6" />}
                    {item.icon === "Pot" && <Sprout className="w-6 h-6 rotate-45" />}
                  </div>
                </div>
              </div>
              ...
              <div className="px-8 pt-10 pb-8 flex-grow flex flex-col justify-between">
                ...
                <div className="mt-8 pt-6 border-t border-dashed border-[#dde8d8] flex items-center justify-between">
                  ...
                  <div className="text-left">
                    <p className="text-xs text-gray-400 font-medium tracking-wide uppercase">Impact Indicator</p>
                    <p className="text-sm font-bold text-[#1f7d2d] mt-0.5">{item.stats.primary}</p>
                  </div>
          ...
          <div className="relative z-10 px-6 sm:px-12 py-16 lg:py-20 flex flex-col lg:flex-row items-center justify-between gap-12">
            ...
            <div className="w-full lg:w-[45%] text-left">
              ...
              <p className="mt-4 text-emerald-100/90 text-sm sm:text-base leading-relaxed max-w-md">
                Be a part of the initiative. Every single plant placed, drop saved, or community inspired today builds a stable ecosystem for a better tomorrow.
              </p>
            ...
            <div className="w-full lg:w-[55%] grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6">
              ...
              <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-5 text-center flex flex-col items-center hover:bg-white/10 transition-colors duration-300">
                ...
                <h4 className="font-bold text-sm text-[#b6ff7a] uppercase tracking-wider">Plant Trees</h4>
                <div className="w-6 h-0.5 bg-[#b6ff7a]/40 my-2"></div>
                <p className="text-xs text-emerald-200/80 leading-relaxed">Grow greenery, restore nature.</p>
              </div>
              ...
              <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-5 text-center flex flex-col items-center hover:bg-white/10 transition-colors duration-300">
                ...
                <h4 className="font-bold text-sm text-[#b6ff7a] uppercase tracking-wider">Save Water</h4>
                <div className="w-6 h-0.5 bg-[#b6ff7a]/40 my-2"></div>
                <p className="text-xs text-emerald-200/80 leading-relaxed">Conserve water, secure future.</p>
              </div>
              ...
              <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-5 text-center flex flex-col items-center hover:bg-white/10 transition-colors duration-300">
                ...
                <h4 className="font-bold text-sm text-[#b6ff7a] uppercase tracking-wider">Reduce Waste</h4>
                <div className="w-6 h-0.5 bg-[#b6ff7a]/40 my-2"></div>
                <p className="text-xs text-emerald-200/80 leading-relaxed">Reduce, Reuse, Recycle.</p>
              </div>
              ...
              <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-5 text-center flex flex-col items-center hover:bg-white/10 transition-colors duration-300">
                ...
                <h4 className="font-bold text-sm text-[#b6ff7a] uppercase tracking-wider">Inspire</h4>
                <div className="w-6 h-0.5 bg-[#b6ff7a]/40 my-2"></div>
                <p className="text-xs text-emerald-200/80 leading-relaxed">Small actions inspire change.</p>
              </div>
